...
- **Use `leanspec` CLI** for all spec operations.
...
- **Using `depends_on` for umbrella child specs** — use parent/child (`leanspec rel add <child> --parent <parent>`) instead.
- **Using parent/child for unrelated blockers** — use `depends_on` (`leanspec rel add <spec> --depends-on <other>`) instead.
